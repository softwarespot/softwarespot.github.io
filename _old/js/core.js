...
App.core = (function coreModule(window, document, $) {
     ...
     */
    var arrayFrom = isFunction(_nativeArrayFrom)
        ? _nativeArrayFrom
        : function arrayFrom(arrayLike, fn, context) {
     ...
     */
    var arrayOf = isFunction(_nativeArrayArrayOf)
        ? _nativeArrayArrayOf
        : function arrayOf() {
        ...
        if (reQuerySelector.test(selector)) {
            ...
            switch (selector[selection]) {
                ...
                case '.':
                    ...
                    return arrayFrom(
                        context.getElementsByClassName(selector.substr(selection).replace(reReplaceDots, ' ')),
                    );
                default:
     ...
     */
    var isArray = isFunction(_nativeArrayIsArray)
        ? _nativeArrayIsArray
     ...
     */
    var isFinite = isFunction(_nativeNumberIsFinite)
        ? _nativeNumberIsFinite
     ...
     */
    var isInteger = isFunction(_nativeNumberIsInteger)
        ? _nativeNumberIsInteger
     ...
     */
    var isNaN = isFunction(_nativeNumberIsNaN)
        ? _nativeNumberIsNaN
     ...
     */
    var isSafeInteger = isFunction(_nativeNumberIsSafeInteger)
        ? _nativeNumberIsSafeInteger
     ...
     */
    var mathSign = isFunction(_nativeMathSign)
        ? _nativeMathSign
     ...
     */
    var mathTrunc = isFunction(_nativeMathTrunc)
        ? _nativeMathTrunc
     ...
     */
    var objectIs = isFunction(_nativeObjectIs)
        ? _nativeObjectIs
    ...
    var stringStripDiacritics = (function stringStripDiacriticsModule() {
        ...
        [
            ...
            },
            {
                base: 'O',
                chars:
                    '\u24C4\uFF2F\xD2\xD3\xD4\u1ED2\u1ED0\u1ED6\u1ED4\xD5\u1E4C\u022C\u1E4E\u014C\u1E50\u1E52\u014E\u022E\u0230\xD6' +
            ...
            },
            {
                base: 'o',
                chars:
                    '\u24DE\uFF4F\xF2\xF3\xF4\u1ED3\u1ED1\u1ED7\u1ED5\xF5\u1E4D\u022D\u1E4F\u014D\u1E51\u1E53\u014F\u022F\u0231' +
    ...
    function stringToKebabCase(value) {
        ...
        return value.replace(_reEscapedCamelCaseChar, '-$1').replace(_reEscapedCaseChars, '-').toLowerCase();
    }
    ...
    function stringToSnakeCase(value) {
        ...
        return value.replace(_reEscapedCamelCaseChars, '$1_$2').replace(_reEscapedCaseChars, '_').toLowerCase();
    }
    ...
    function stringTrimLeft(value, count) {
        return isString(value) && isInteger(count) && count > 0 && count < value.length
            ? value.slice(count)
            : STRING_EMPTY;
    }
    ...
    function stringTrimRight(value, count) {
        return isString(value) && isInteger(count) && count > 0 && count < value.length
            ? value.slice(0, value.length - count)
            : STRING_EMPTY;
    }
    ...
    function toISOString(date) {
        ...
        return (
            ...
            '-' +
            stringPad(date.getUTCDate(), STRING_PADDING_CHAR, 2) +
            'T' +
            ...
            '.' +
            stringPad(date.getUTCMilliseconds(), STRING_PADDING_CHAR, 3) +
            'Z'
        );
    }
     ...
     */
    function type(value) {
        var tag = _nativeObjectToString.call(value).replace(_reTypeOf, '$1').toLowerCase();
    ...
    function _trim(value) {
        return isFunction(_nativeStringTrim) ? _nativeStringTrim.call(value) : value.replace(_reTrim, STRING_EMPTY);
    }
