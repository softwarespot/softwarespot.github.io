...
gulp.task('clean', function cleanTask(cb) {
    del([Assets.css.dest + '/**/*.*', Assets.images.dest + '/**/*.*', Assets.js.dest + '/**/*.*'], cb);
});

// Check the main css file(s) meets the following standards outlined in .stylelintrc
gulp.task('csslint', function cssLintTask() {
    return gulp.src(Assets.css.custom.all).pipe(
        styleLint({
            reporters: [styleLintConsoleReporter()],
        }),
    );
});
...
gulp.task('htmlmin', function cssMinTask() {
    ...
    return gulp.src(all).pipe(htmlMin(_htmlSettings)).pipe(gulp.dest(dest));
});
